...
def load_base_excel(file) -> pd.DataFrame:
    def _need(col):
        ...
        return (str(col) in base_cols) or (n in {"P9","P09","PERIOD9"})
...
def build_weekly(df: pd.DataFrame, series_cols: List[str]) -> pd.DataFrame:
    ...
    wk_full = (full.merge(wk, on=group_cols, how="left")
...
def ml_backtest_v2(
    ...
    for keys, gtr in train_df.groupby(series_cols, dropna=False):
        # validation subset for the same series
        gva = val_df.copy()
...
try:
    ...
    with tab1:
        ...
        # build a 3-mode comparison for chart/table
